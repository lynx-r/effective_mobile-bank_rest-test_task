...
Тесты для сервиса управления транзакциями (`TransactionServiceImpl`).

**Покрываемые методы:**
- `transferBetweenOwnCards(String username, InternalTransferRequest request)` - перевод денег между картами пользователя

**Успешные сценарии (4 теста):**
- ✅ Успешный перевод между картами пользователя
- ✅ Проверка корректности изменения балансов
- ✅ Проверка сохранения записи транзакции
- ✅ Проверка полей транзакции (fromCard, toCard, amount, status, description)

**Тесты валидации и ошибок (4 теста):**
- ✅ Недостаточно средств на карте списания
- ✅ Карта списания заблокирована
- ✅ Карта списания не найдена
- ✅ Карта зачисления не найдена

**Edge Cases и граничные случаи (6 тестов):**
- ✅ Перевод на ту же карту (fromCardId == toCardId)
- ✅ Перевод с нулевой суммой
- ✅ Перевод с отрицательной суммой
- ✅ Перевод с очень большой суммой
- ✅ Попытка перевода с карты другого пользователя
- ✅ Обработка ошибок репозитория

**Тесты производительности (2 теста):**
- ✅ Последовательные переводы (конкурентность)
- ✅ Массовые операции с множественными переводами
...
- **Общее количество тестов: 57** (было 41, стало 57)
  - UserServiceImplTest: 17 тестов (было 5, стало 17)
  - CardServiceImplTest: 24 теста (было 7, стало 24)
  - TransactionServiceImplTest: 16 тестов (было 0, стало 16)
- **Время выполнения:** ~45-60 секунд (увеличение из-за дополнительных тестов)
- **Покрытие:** 100% публичных методов всех сервисов + комплексное покрытие edge cases
